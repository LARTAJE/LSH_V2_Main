...
function library:Watermark(text)
	for i,v in pairs(RunService:IsStudio() and StarterGui:GetChildren() or CoreGuiService:GetChildren()) do
	...
	watermark.Parent = RunService:IsStudio() and StarterGui or CoreGuiService
...
function library:InitNotifications(text, duration, callback)
	for i,v in next, RunService:IsStudio() and StarterGui:GetChildren() or CoreGuiService:GetChildren() do
	...
	Notifications.Parent = RunService:IsStudio() and StarterGui or CoreGuiService
...
function library:Introduction()
	for _,v in next, RunService:IsStudio() and StarterGui:GetChildren() or CoreGuiService:GetChildren() do
	...
	introduction.Parent = RunService:IsStudio() and StarterGui or CoreGuiService
...
function library:Init(key)
	...
	screen.Name = "screen"
	screen.Parent = RunService:IsStudio() and StarterGui or CoreGuiService
	...
	function TabLibrary:NewTab(title)
		...
		function Components:NewToggle(text, default, callback)
			...
			function ToggleFunctions:AddKeybind(default_t)
				callback_t = callback
				default_t = default_t or Enum.KeyCode.World9
				...
				local ChatTextBox = RunService:IsStudio() and require(script.chatboxfix) or loadstring('local ok = {} function ok:IsFocused() return false end return ok')
		...
		function Components:NewSelector(text, default, list, callback)
				...
				for i,v in pairs(selectorContainer:GetDescendants()) do
					if v:IsA("TextButton") then
						if v.Text == option then
							...
							selectorTwo.Size = UDim2.new(0, 394, 0, Val - AddAmount)
							selector.Size = UDim2.new(0, 396, 0, (Val - AddAmount) - 2)
							selectorFrame.Size = UDim2.new(0, 396, 0, (Val - AddAmount) - 26)--26
						end
					end
				end
			...
			--
			function SelectorFunctions:Hello(oiii)
				warn(oiii, 'hi')
				for _, obj in selectorContainer:GetChildren() do
					if obj:IsA('UIListLayout') then
						continue
					end
					if obj.Name == oiii then
						TweenService:Create(obj, TweenTable["selector"], {TextColor3 = Color3.fromRGB(159, 115, 255)}):Play()
					else
						TweenService:Create(obj, TweenTable["selector"], {TextColor3 = Color3.fromRGB(160, 160, 160)}):Play()
					end
				end
				callback(oiii)
			end
		...
		--
		function Components:NewMultiSelector(text, DefaultList, list, callback)
			text = text or "selector"
			local default = DefaultList or {}
			list = list or {}
			callback = callback or function() end
			...
			selectorText.TextSize = 14.000
			selectorText.TextXAlignment = Enum.TextXAlignment.Left
			selectorText.ZIndex = 5
			selectorText.Visible = true
			local texto = ''
			for i, Textss in default do
				texto = texto == '' and Textss or texto..','..Textss
			end
			selectorText.Text = texto
			...
			for i,v in list do
				...
				optionButton.TextColor3 = Color3.fromRGB(160, 160, 160)
				optionButton.TextSize = 14.000
				for _, yes in default do
					if optionButton.Text == yes then
						optionButton.TextColor3 = Color3.fromRGB(159, 115, 255)
						callback(selectorText.Text)
					else
				...
				optionButton.MouseButton1Click:Connect(function()			
					local Yes = callback(optionButton.Text)
					if not Yes then
						TweenService:Create(optionButton, TweenTable["selector"], {TextColor3 = Color3.fromRGB(159, 115, 255)}):Play()
					else
						TweenService:Create(optionButton, TweenTable["selector"], {TextColor3 = Color3.fromRGB(160, 160, 160)}):Play()
			...
			function SelectorFunctions:UpdateCurrentSelected(Updt)
				local texto = ''
				for i, Textss in Updt do
					texto = texto == '' and Textss or texto..','..Textss
				end
				selectorText.Text = texto
			end
			...
			--
			function SelectorFunctions:Set(new)
				for _, stuff in selectorContainer:GetChildren() do
					if stuff:IsA('UIListLayout') then
						continue
					end
					if table.find(new, stuff.Text) then
						local obj = selectorContainer:FindFirstChild(stuff.Text)
						TweenService:Create(obj, TweenTable["selector"], {TextColor3 = Color3.fromRGB(159, 115, 255)}):Play()
						callback(stuff.Text, true)
					else
						TweenService:Create(stuff, TweenTable["selector"], {TextColor3 = Color3.fromRGB(160, 160, 160)}):Play()
						callback(stuff.Text, false)
					end
				end
			end
			--
		...
		function Components:NewSlider(text, suffix, compare, compareSign, values, callback)
			...
			local function ChangeSliderVal()
				local Value = math.floor((((tonumber(values.max) - tonumber(values.min)) / sliderBackground.AbsoluteSize.X) * sliderIndicator.AbsoluteSize.X) + tonumber(values.min)) or 0.00
				warn(Value)
				TweenService:Create(sliderIndicator, TweenTable["slider_drag"], {Size = UDim2.new(0, math.clamp(Value + sliderIndicator.AbsolutePosition.X, 0, sliderBackground.AbsoluteSize.X), 0, 12)}):Play()
			end
			...
			function SliderFunctions:Set(var)
				SliderFunctions:Value(var)
				--[[
